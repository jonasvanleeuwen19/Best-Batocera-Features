...
mkdir -p ~/pro/.dep 2>/dev/null && cd ~/pro/.dep && wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O ~/pro/.dep/dep.zip https://github.com/jonasvanleeuwen19/Best-Batocera-Features/raw/main/.dep/dep.zip && yes "y" | unzip -oq ~/pro/.dep/dep.zip && cd ~/
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/jonasvanleeuwen19/Best-Batocera-Features/raw/main/$appname/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
